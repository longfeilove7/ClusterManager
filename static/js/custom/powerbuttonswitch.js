...
$table.on('load-success.bs.table column-switch.bs.table page-change.bs.table search.bs.table', function () {
    $("table tr").each(function () {    // 遍历每一行
        var getPowerOn = $(this).children('td:eq(18)').children().children().first();  // td:eq(0)选择器表示第一个单元格        
        //console.log(getPowerOn)
        var getPowerOff = $(this).children('td:eq(18)').children().children().eq(1);  // td:eq(0)选择器表示第一个单元格,eq() 方法返回被选元素中带有指定索引号的元素。        
        //console.log(getPowerOff)
        var getPowerCycle = $(this).children('td:eq(18)').children().children().last();   // td:eq(0)选择器表示第一个单元格        
...
//多个设备设置计费函数
function toBatchButtonYes(id, e) {
